...
export class AjoutproduitComponent {
  ...
  onFileSelected(event:any){
    ...
    if(fileList){
      ...
      this.previewImge(this.imageToUpload);
    }
  }
  onSubmit()
  {
    if(this.produitForm.valid && this.imageToUpload){
      ...
      this.produitService.createProduit(this.produit, this.imageToUpload).subscribe(
        {
          next:()=>{
            this.error="";
          ...
          {if(error.status===400)
          {
            ...
            this.error='Erreur los de l\'ajout du produit';
          }
        }
      )
    }
